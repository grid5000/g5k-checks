...
Ohai.plugin(:NetworkAdapters) do
  ...
  collect_data do
    ...
    interfaces.reject{ |d,i| %w{ ib }.include?(i[:type]) || d == "lo" }.each do |dev, iface|
      ...
      iface[:name] = Utils.shell_out("/sbin/udevadm test 2>&1 /sys/class/net/#{dev} | grep ID_NET_NAME_PATH | cut -d'=' -f2").stdout.chomp() rescue ""

      if iface[:name].nil? || iface[:name].empty?
        iface[:name] = dev
      end
      ...
      while Time.now.to_i < now + timeout
        newStatus = Utils.interface_operstate(dev)
        if ifaceStatus == "up" || newStatus == "up"
          ethtool_infos = Utils.interface_ethtool(dev)
          if (!ethtool_infos[:rate].nil? && !ethtool_infos[:rate].empty?)
            break
          end
        end
        sleep(1)
