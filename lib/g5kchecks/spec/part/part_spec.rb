describe "Partitions" do
 ...
 Utils.layout.each do |k,v|
  ...
  it "should have the correct end" do
    ...
    end_lay = v[:end]
    end_lay.should eql(end_sys), "filesystem layout end of #{k} is #{end_sys} instead of #{end_lay}"
  end
  ...
  it "should have the correct state" do
    ...
    state_sys.should eql("clean"), "filesystem partition state of #{@system["layout"]["#{k}"]} is #{@system["layout"]["#{k}"][:state]}"
  end
 ...
 Utils.fstab.each do |k,v|
  ...
  v["options"].each_with_index do |o,i|
    ...
    it "should have the correct options" do
      ...
      opt.should eql(true), "filesystem fstab option #{o} is not present #{@system[k]["mount_options"].to_s}"
    end
