describe "TestDisk" do
  ...
  RSpec.configuration.node.ohai_description.block_device.select { |key,value| key =~ /[sh]da/ and value["model"] != "vmDisk" }.each { |k,v|
    ...
    it "should give good results in read" do
      POpen4::popen4("fio #{File.dirname(__FILE__) + "/../../data/read.fio"}") do |stdout, stderr, stdin, pid|
      stdin.close
        stdout.each do |line|
          if line =~ /READ/
    ...
    it "should give good results in write" do
      POpen4::popen4("fio #{File.dirname(__FILE__) + "/../../data/write.fio"}") do |stdout, stderr, stdin, pid|
      stdin.close
        stdout.each do |line|
          if line =~ /WRITE/
