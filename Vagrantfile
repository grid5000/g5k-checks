...
Vagrant.configure("2") do |config|
  ...
  # Curl - called by vagrant - has an issue with certificates from irisa.fr, ignore it
  ...
  #config.vm.network :forwarded_port, guest: 19876, host: 29876
  ...
  #config.vm.provision "file", source: "/home/nmichon/.ssh/config", destination: ".ssh/config"

  # Copy the production-like apache2 vhost configuration file
  #config.vm.provision "file", source: "config/apache_ums.conf", destination: "/etc/apache2/sites-enabled/ums.conf"
  ...
  # Run stuff after services where provisionned with puppet
